...
var contactList = [];
const baseUrl = 'http://18.191.145.56:5004';
...
function formatMatches(json) {
    ...
    var result = '<table class="table table-success table-striped""><tr><th>First</th><th>Last</th><th>Phone</th><th>Email</th><th>Age</th><th>Type</th><th>Action</th><tr>';
    json.forEach(function(entry, i) {
        result += "<tr><td class='first'>" + entry['first'] + "</td><td class='last'>" + entry['last'];
        result += "</td><td class='phone'>" + entry['phone'] + "</td><td class='type'>" + entry['email'];
        result += "</td><td class='phone'>" + entry['age'] + "</td><td class='type'>" + entry['type'] + "</td>";
...
function addContact() {
    ...
    $('#searchresults').empty();
    fetch(baseUrl + '/contact/add/' + $('#addfirst').val() + "/" + $('#addlast').val() + "/" + $('#addphone').val()+ "/" + $('#addemail').val()+ "/" + $('#addage').val() + "/" + $('#addtype').text(), {
            method: 'get'
        })
        .then(response => response.json())
...
function updateContact() {
    ...
    fetch(baseUrl + '/contact/update/' + id + '/' + document.getElementById("editfirst").value 
    		+ '/' + document.getElementById("editlast").value + '/' + document.getElementById("editphone").value + '/' + document.getElementById("editemail").value 
    		+ '/' + document.getElementById("editage").value+ '/' + document.getElementById("edittype").innerText, {
                method: 'get'
            })
