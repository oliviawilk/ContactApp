...
int main(){
	...
	results = ctdb.findByFirst(firstMatch);
        ...
        string first,last,phone,email,age,type;
        getline(cin,first);
        ...
        ctdb.addEntry(first,last,phone,email,age,type);
        cout << "Entry added" << endl;

      } else if (o=="5") {
        string first,last,phone,email,age,type, IDnum;
        ...
        if (entry.ID.empty()) {
        	...
        	cout << "Enter a new email address("+entry.email+"): ";
        	getline(cin,email);
        	...
        	ctdb.editEntry(entry.ID,entry.first,entry.last,entry.phone,entry.email,entry.age,entry.type);
        	cout << "Done!" << endl;
        }
      } else if (o=="6") {
