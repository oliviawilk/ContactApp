...
vector<contactEntry> contactDB::find(string search) {
    ...
    while (res->next()) {
    	contactEntry entry(res->getString("First"),res->getString("Last"),
			res->getString("Phone"), res->getString("Email"), res->getString("Age"),res->getString("Type"),
	    	res->getString("ID"));
...
void contactDB::addEntry(string first,string last,string phone, string email, string age, string type){
	...
	if (!conn) {
  	...
  	stmnt->executeQuery("INSERT INTO contacts(First,Last,Phone, Email, Age,Type) VALUES ('"+first+"','"+last+"','"+phone+"','"+email+"','"+age+"','"+type+"')");
}
...
void contactDB::editEntry(string idnum,string first,string last,string phone, string email, string age, string type){
	if (!conn) {
   		cerr << "Invalid database connection" << endl;
   		exit (EXIT_FAILURE);
  	}
  	...
  	stmnt->executeQuery("UPDATE contacts SET First = '"+first+"', Last ='"+last+"', Phone ='"+phone+"', Email ='"+email+"', Age ='"+age+"', Type ='"+type+"' WHERE ID='"+idnum+"'");
